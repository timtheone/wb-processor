...
export async function get_single_supply_pdf(bot: Telegraf<MyContext<Update>>) {
  bot.command("generate_single_supply_pdf", async (ctx) => {
    ...
    const supplyIdsPromises = shops.map(async (shop) => {
      return apiClient
        .getLastSupply(shop.token)
        .then((response) => ({
          status: "fulfilled",
          supplyIds: [response.lastSupplyId],
          shopName: shop.name,
        }))
